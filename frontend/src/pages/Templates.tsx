...
export default function Templates() {
  ...
  const submitAdd = async (e: React.FormEvent) => {
    ...
    try {
      ...
      data.append('system_prompt', formData.system_prompt.trim());
      if (file) {
      ...
      showToast('模板创建成功');
      setShowAdd(false);
      setFormData({ name: '', content: '', is_default: false, system_prompt: '' });
      setFile(null);
      fetchData();
    } catch (e) {
    } finally {
  ...
  return (
    <>
      ...
      {/* 新建模板模态框 */}
      {showAdd && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-[1000] flex justify-center items-center animate-modal-in" onClick={() => setShowAdd(false)}>
          <div className="bg-white border border-slate-200 rounded-2xl p-7 w-[90%] max-w-[640px] max-h-[85vh] flex flex-col shadow-[0_20px_60px_rgba(0,0,0,0.5)]" onClick={(e) => e.stopPropagation()}>
            ...
            <form onSubmit={submitAdd} className="flex flex-col flex-1 overflow-hidden">
              <div className="flex-1 overflow-y-auto pr-1 mb-5">
                ...
                <div className="mb-5">
                  <label className="block text-[13px] font-semibold text-slate-500 mb-1.5">AI汇总系统设定提示词 (System Prompt - 可选)</label>
                  <p className="text-xs text-slate-500 mb-2">在此处可为此模板定义专属的汇总助手系统指令。不填则使用系统默认设定。</p>
                  <textarea className={`${inputClass} min-h-[120px] font-mono`} 
                    placeholder="请输入大模型汇总的系统指令设定..."
                    value={formData.system_prompt} onChange={e => setFormData({...formData, system_prompt: e.target.value})}></textarea>
                </div>
